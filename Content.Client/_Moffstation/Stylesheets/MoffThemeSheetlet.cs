...
namespace Content.Client._Moffstation.Stylesheets;

/// <summary>
/// Fork-wide re-theme. Deliberately has no <see cref="CommonSheetletAttribute"/> so that the stylesheets can invoke
/// it explicitly after every other sheetlet, letting these rules win ties against upstream ones.
/// </summary>
public sealed class MoffThemeSheetlet : Sheetlet<PalettedStylesheet>
{
    // Surfaces
    private static readonly Color WindowBackground = Color.FromHex("#262626");
    private static readonly Color WindowBorder = Color.FromHex("#0b0d0f");
    private static readonly Color TitleBarBackground = Color.FromHex("#363636");
    private static readonly Color SectionBackground = Color.FromHex("#202428");
    private static readonly Color SectionBackgroundLight = Color.FromHex("#292e33");
    private static readonly Color SectionBackgroundDark = Color.FromHex("#1d2227");

    // Accents
    private static readonly Color AccentBlue = Color.FromHex("#477cab");
    private static readonly Color AccentBlueHover = Color.FromHex("#6394bb");
    private static readonly Color AccentBluePressed = Color.FromHex("#31516e");

    // Text
    private static readonly Color TextNormal = Color.FromHex("#E1E1E1");
    private static readonly Color TextMuted = Color.FromHex("#8997a3");

    // Status
    private static readonly Color PositiveGreen = Color.FromHex("#1B9638");
    private static readonly Color NegativeRed = Color.FromHex("#BD2020");
    ...
    public override StyleRule[] GetRules(PalettedStylesheet sheet, object config)
    {
        ...
        var divider = new StyleBoxFlat { BackgroundColor = AccentBlue };
        ...
        var panel = MakePanel(SectionBackground, Color.FromHex("#30363b"));
        var panelLight = MakePanel(SectionBackgroundLight, Color.FromHex("#3a4249"));
        var panelDark = MakePanel(SectionBackgroundDark, WindowBorder);
        ...
        [
            // Windows
            E().Class(DefaultWindow.StyleClassWindowPanel).Panel(window),
            E().Class(DefaultWindow.StyleClassWindowHeader).Panel(titleBar),
            E().Class(StyleClass.BorderedWindowPanel).Panel(window),
            E().Class(StyleClass.AlertWindowHeader).Panel(alertTitleBar),
            E<PanelContainer>().Class(MoffStyleClass.WindowBackground).Panel(window),
            E<PanelContainer>().Class(MoffStyleClass.WindowTitleBar).Panel(titleBar),
            E<PanelContainer>().Class(MoffStyleClass.WindowTitleDivider).Panel(divider),
            ...
            // Panels
            E().Class(StyleClass.BackgroundPanel).Panel(panel),
            E().Class(StyleClass.BackgroundPanelDark).Panel(panelDark),
            E().Class(StyleClass.BackgroundPanelOpenLeft).Panel(panel),
            E().Class(StyleClass.BackgroundPanelOpenRight).Panel(panel),
            ...
            // Output and chat surfaces
            E<OutputPanel>().Panel(outputBackground),
            E<PanelContainer>().Class(ChatInputBox.StyleClassChatPanel).Panel(chatBackground),
            E<PanelContainer>().Class("StyleNano.StyleClassChatPanel").Panel(chatBackground),

            // Dividers
            E<PanelContainer>().Class(StyleClass.LowDivider).Panel(divider),
            E<PanelContainer>().Class(StyleClass.HighDivider).Panel(divider),
            E<HLine>().Class(StyleClass.Highlight).Panel(divider),
            ...
            E<LineEdit>()
                ...
                .Prop(LineEdit.StylePropertyStyleBox, inputReadOnly),
            E<LineEdit>().Class(LineEdit.StyleClassLineEditNotEditable).FontColor(TextMuted),
            E<LineEdit>().Pseudo(LineEdit.StylePseudoClassPlaceholder).FontColor(TextMuted),
            E<TextEdit>().Pseudo(TextEdit.StylePseudoClassPlaceholder).FontColor(TextMuted),

            // Dropdowns
            E<PanelContainer>().Class(OptionButton.StyleClassOptionsBackground).Panel(optionPopup),
            E<Label>()
            ...
            // Scrollbars
            E<VScrollBar>().Prop(ScrollBar.StylePropertyGrabber, scrollNormal),
            E<VScrollBar>().PseudoHovered().Prop(ScrollBar.StylePropertyGrabber, scrollHovered),
            E<VScrollBar>().PseudoPressed().Prop(ScrollBar.StylePropertyGrabber, scrollPressed),
            E<HScrollBar>().Prop(ScrollBar.StylePropertyGrabber, scrollNormal),
            E<HScrollBar>().PseudoHovered().Prop(ScrollBar.StylePropertyGrabber, scrollHovered),
            E<HScrollBar>().PseudoPressed().Prop(ScrollBar.StylePropertyGrabber, scrollPressed),

            // Switch buttons
            E<SwitchButton>().Prop(SwitchButton.StylePropertySeparation, 8),
            E<SwitchButton>()
            ...
            E<Label>()
                ...
                .FontColor(TextNormal),
            E<Label>().Class(StyleClass.LabelSubText).FontColor(TextMuted),
            E<Label>().Class(StyleClass.LabelWeak).FontColor(TextMuted),
            E<Label>().Class(StyleClass.Highlight).FontColor(Color.FromHex("#a9c9e5")),

            // Tooltips
            E<Tooltip>().Panel(tooltip),

            // Headings
            E<NanoHeading>().ParentOf(E<PanelContainer>()).Panel(nanoHeading),
        ];
    }
    ...
    private static StyleBoxFlat MakeListItem(Color background)
    {
        return new StyleBoxFlat
        {
            BackgroundColor = background,
    ...
    private static StyleBoxFlat MakeScrollGrabber(Color background)
    {
        return new StyleBoxFlat
        {
            BackgroundColor = background,
