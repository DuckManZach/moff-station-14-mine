...
/// <summary>Surfaces used by the lobby screen.</summary>
[CommonSheetlet]
public sealed class MoffLobbySheetlet : Sheetlet<PalettedStylesheet>
{
    public override StyleRule[] GetRules(PalettedStylesheet sheet, object config)
    {
        ...
        // The lobby chat sits inside our own panel, so strip the chrome the global chat style adds.
        ...
        return
        [
            E<PanelContainer>().Class(MoffStyleClass.LobbyRoot).Panel(root),
            E<PanelContainer>().Class(MoffStyleClass.LobbyTopBar).Panel(topBar),
            E<PanelContainer>().Class(MoffStyleClass.LobbyToolbar).Panel(toolbar),
            E<PanelContainer>().Class(MoffStyleClass.LobbyCard).Panel(card),
            E<PanelContainer>().Class(MoffStyleClass.LobbyCardHeader).Panel(cardHeader),
            E<PanelContainer>().Class(MoffStyleClass.LobbyChat).Panel(chat),
            E<PanelContainer>().Class(MoffStyleClass.LobbyFloating).Panel(floating),

            E<Label>()
                .Class(MoffStyleClass.LobbyTitle)
                .Font(sheet.BaseFont.GetFont(16, FontKind.Bold))
                .FontColor(Color.FromHex("#e3e9ee")),
            E<Label>()
                .Class(MoffStyleClass.LobbyCardTitle)
                .Font(sheet.BaseFont.GetFont(14, FontKind.Bold))
                .FontColor(Color.FromHex("#e3e9ee")),
            E<Label>().Class(MoffStyleClass.LobbyMuted).FontColor(Color.FromHex("#95a1aa")),

            E<ChatBox>()
                .ParentOf(E<PanelContainer>().Class(ChatInputBox.StyleClassChatPanel))
                .Panel(lobbyChatInner),
            E<ChatBox>()
                .ParentOf(E<PanelContainer>().Class("StyleNano.StyleClassChatPanel"))
                .Panel(lobbyChatInner),
        ];
    }
}
